...
const participants = participantsRaw.map((p: any) => ({
  ...
  user_id: p.user_id?._id.toString(), // <-- ajouter ceci
}));
...
export async function POST(request: Request, { params }: { params: any }) {
  ...
  // Vérifie si le tournoi est passé
  ...
  // Vérifie le nombre maximal de participants
  ...
  const alreadyRegistered = await ParticipantModel.findOne({
    tournament_id: id,
    user_id: decoded.id,
  });
  if (alreadyRegistered) {
    return NextResponse.json({ error: "Vous êtes déjà inscrit à ce tournoi" }, { status: 400 });
  }
